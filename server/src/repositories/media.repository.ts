...
export class MediaRepository {
  ...
  async extract(input: string): Promise<ExtractResult | null> {
    return this.storageRepository.withLocalPath(input, async (localPath) => {
      try {
        const buffer = await exiftool.extractBinaryTagToBuffer('JpgFromRaw2', localPath);
      ...
      try {
        const buffer = await exiftool.extractBinaryTagToBuffer('JpgFromRaw', localPath);
        return { buffer, format: RawExtractedFormat.Jpeg };
      } catch (error: any) {
      ...
      try {
        const buffer = await exiftool.extractBinaryTagToBuffer('PreviewJXL', localPath);
      ...
      try {
        const buffer = await exiftool.extractBinaryTagToBuffer('PreviewImage', localPath);
        return { buffer, format: RawExtractedFormat.Jpeg };
      } catch (error: any) {
  ...
  async writeExif(tags: Partial<Exif>, output: string): Promise<boolean> {
    return this.storageRepository.withLocalPath(output, async (localPath) => {
      try {
        ...
        await exiftool.write(localPath, tagsToWrite, {
  ...
  async copyTagGroup(tagGroup: string, source: string, target: string): Promise<boolean> {
    return this.storageRepository.withLocalPath(source, async (localSource) =>
      this.storageRepository.withLocalPath(target, async (localTarget) => {
        try {
          await exiftool.write(
            ...
            {
              ignoreMinorErrors: true,
              writeArgs: ['-TagsFromFile', localSource, `-${tagGroup}:all>${tagGroup}:all`, '-overwrite_original'],
            },
          );
          return true;
        } catch (error: any) {
  ...
  async decodeImage(input: string | Buffer, options: DecodeToBufferOptions) {
    ...
    // For file paths, use withLocalPath to handle S3
    return this.storageRepository.withLocalPath(input, async (localPath) => {
      return this.getImageDecodingPipeline(localPath, options).raw().toBuffer({ resolveWithObject: true });
    });
  }

  async generateThumbnail(input: string | Buffer, options: GenerateThumbnailOptions, output: string): Promise<void> {
    if (Buffer.isBuffer(input)) {
      await this.storageRepository.writeFile(output, async (outputPath) => {
    ...
    // For file paths, use withLocalPath to handle S3 input
    await this.storageRepository.withLocalPath(input, async (localInputPath) => {
      await this.storageRepository.writeFile(output, async (outputPath) => {
        await this.getImageDecodingPipeline(localInputPath, options)
          .toFormat(options.format, {
            quality: options.quality,
            chromaSubsampling: options.quality >= 80 ? '4:4:4' : '4:2:0',
          })
